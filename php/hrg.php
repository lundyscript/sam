...
<h4 class="display-4 text-center" style="font-size: 26pt">HARGA JUAL</h4>
<hr />
<center>
...
<?php
  ...
  for($i=0; $i<count($arraynomer); $i++){
    $query = mysql_query("SELECT *FROM harga WHERE no='$arraynomer[$i]'") or die ('Error!!'.mysql_error());
    $row = mysql_fetch_array ($query);
    extract($row);
    $no = $i+1;
    echo "
      <tr>
        <td><a href='hrge.php?idh=$idh'><img src='../img/doc.png' width='20'/></a></td>
        <td>".$no."</td>
        <td>".$tipe." ".$ket."</td>

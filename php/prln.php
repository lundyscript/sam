...
<h4 class="display-4 text-center" style="font-size: 26pt">KAS LUNAS</h4>
<hr />
<form method="post">
...
if(isset ($_POST['cari'])){
  ...
  <thead>
    <tr>
      ...
      <th scope="col" width="15%">TANGGAL LUNAS</th>
  ...
  <?php
  $arraytotal = array();
  $query = mysql_query("SELECT *FROM penjualan WHERE status='Lunas' AND tgllunas>='$txttgl1' AND tgllunas<='$txttgl2' ORDER BY tgllunas") or die ('Error!!'.mysql_error());
  while ($row = mysql_fetch_array ($query)) {
    extract($row);
    $arr       = explode('-', $tglorder);
    $tglorder  = $arr[2].'/'.$arr[1].'/'.$arr[0];
    $arr       = explode('-', $tgllunas);
    $tgllunas  = $arr[2].'/'.$arr[1].'/'.$arr[0];
  ...
  $arraytotal  = array();
  $query1 = mysql_query("SELECT *FROM penjualan WHERE status='Lunas' AND tgllunas>='$txttgl1' AND tgllunas<='$txttgl2' ORDER BY tglorder") or die ('Error!!'.mysql_error());
